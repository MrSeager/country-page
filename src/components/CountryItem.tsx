...
const CountryItem: FC<CountryItemProps> = ({ country, index, handleShowInfo }) => {
    ...
    return (
        ...
        >
            ...
            />
            <Col lg={2} xs={2} className='ps-0 z-1'>
                <Image fluid src={country.flags.svg} alt={country.flags.alt} className='cs-flag-img rounded' />
            </Col>
            <Col lg={4} md={4} xs={5} className='overflow-x-auto p-0 d-flex align-items-center z-1'>
                <h3 className='h5 m-0 text-nowrap'>{country.name.common}</h3>
            </Col>
            <Col lg={2} md={4} xs={5} className='overflow-x-auto p-0 d-flex align-items-center z-1'>
                <h3 className='h5 m-0'>{country.population.toLocaleString()}</h3>
            </Col>
            <Col lg={2} md={2} xs={0} className='p-0 d-none d-md-flex align-items-center z-1'>
                <h3 className='h5 m-0'>{country.area.toLocaleString()}</h3>
            </Col>
            <Col lg={2} xs={0} className='p-0 d-none d-lg-flex align-items-center z-1'>
